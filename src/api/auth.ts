...
async function fetchWhoAmI(): Promise<WhoAmIResponse> {
  ...
  return {
    ...
    accounts: ((adapted.accounts as AccountMembership[] | undefined) ?? []).map(
      (a) => ({
...
export function extractDenyReasons(details: unknown): DenyReason[] {
  ...
  for (const d of details) {
    ...
    const raw = (md as { deny_reasons?: unknown; denyReasons?: unknown })
      .deny_reasons ?? (md as { denyReasons?: unknown }).denyReasons;
    ...
    for (const r of raw) {
      ...
      out.push({
        ...
        required_relation:
          (rr.required_relation as string | undefined) ??
          (rr.requiredRelation as string | undefined),
