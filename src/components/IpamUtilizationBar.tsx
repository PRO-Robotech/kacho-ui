...
export function IpamUtilizationBar({ total, used, free, percent, label, className }: Props) {
  ...
  const pct = percent !== undefined
    ...
    : totalN > 0 ? Math.floor((usedN * 100) / totalN) : 0;

  const color = pct >= 90 ? "bg-red-500"
  ...
  return (
    <div className={className}>
      ...
      <div className="flex gap-4 text-xs text-muted-foreground mt-1">
        <span>used: <span className="font-mono text-foreground">{usedN}</span></span>
        <span>free: <span className="font-mono text-foreground">{freeN}</span></span>
        <span>total: <span className="font-mono text-foreground">{totalN}</span></span>
      </div>
    </div>
  );
}
