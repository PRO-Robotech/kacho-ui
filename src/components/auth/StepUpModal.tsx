...
export function StepUpModal() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const initRes = await fetch(
        `${config.kratosUrl}/self-service/login/browser?refresh=true&aal=aal2`,
  ...
  return (
    ...
    >
      <Paragraph>
        Эта операция требует дополнительной проверки безопасности (ACR={acr}).
      </Paragraph>
      <Paragraph>
        <Text type="secondary">
          Подтвердите запрос вашим passkey с биометрией (Touch&nbsp;ID / Windows&nbsp;Hello /
          security key).
        </Text>
      </Paragraph>
      {error && (
