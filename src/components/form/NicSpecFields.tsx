...
export function NicSpecFields({ pathPrefix, value, onChange }: Props) {
  ...
  const subnets = subnetsQ.data?.subnets ?? [];
  const allAddresses = addressesQ.data?.addresses ?? [];
  const internalAddrs = allAddresses.filter((a) => !!a.internal_ipv4_address);
  const externalAddrs = allAddresses.filter((a) => !!a.external_ipv4_address);
    ...
    <div className="space-y-4">
      ...
      {!useExisting && (
        <>
          ...
          <div className="space-y-1.5">
            <Label required description="Уровень 1 — сеть (с поиском); уровень 2 — подсеть; уровень 3 — внутренний IPv4-адрес из подсети, либо «(без адреса)», либо «+ Создать адрес…».">
              Сеть и адрес
            </Label>
            <Cascader
              style={{ width: "100%" }}
            ...
            {subnetId && (
              <div className="text-xs text-muted-foreground space-y-0.5">
                <div>Подсеть: <CopyableId id={subnetId} /></div>
                ...
                {extAddrId && (() => {
                  ...
                  return (
                    <div className="text-xs text-muted-foreground">
                      {a?.external_ipv4_address?.address ? <span className="font-mono">{a.external_ipv4_address.address}</span> : null}{" "}
