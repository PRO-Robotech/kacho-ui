  ...
  subnets: {
    ...
    fields: [
      ...
      },
      {
        ...
        type: "array",
        itemLabel: "CIDR",
        description: "Опционально. IPv6 CIDR-блоки подсети (settable только при создании).",
        // В Edit поле не показывается — UpdateSubnet.v6_cidr_blocks soft-immutable
        // (поле принимается, но изменение значения не применяется; см.
        // applySubnetMask), поэтому в Edit-форме оно вводило в заблуждение.
        // После Create управляется через SubnetCidrManager на DetailPage
        // (verbs :add-cidr-blocks / :remove-cidr-blocks), как и v4_cidr_blocks.
        editHidden: true,
        newItem: () => ({ value: "" }),
        itemFields: [
          {
