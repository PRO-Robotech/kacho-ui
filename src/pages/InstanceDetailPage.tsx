...
export function InstanceDetailPage() {
  ...
  const secondaryActions = useMemo(
    () => (data: Record<string, unknown>) => {
      ...
      return (
        <Space size={8} wrap>
          <Button icon={<PlusOutlined />} onClick={() => { setAttachDiskId(undefined); setAutoDelete(false); setAttachOpen(true); }}>
            Подключить диск
          </Button>
          <Button
            icon={<MinusOutlined />}
            disabled={secondaryIds.length === 0}
            onClick={() => { setDetachDiskId(secondaryIds[0]); setDetachOpen(true); }}
          >
            Отключить диск
          </Button>
          {bootDiskId && (
