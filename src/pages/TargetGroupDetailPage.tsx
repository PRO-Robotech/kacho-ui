...
export function buildTargetPayload(kind: TargetKind, f: TargetFormState): Target | null {
  ...
  switch (kind) {
    ...
    case "ip_ref":
      return f.subnetId && f.ipAddr
        ? { ip_ref: { subnet_id: f.subnetId, address: f.ipAddr }, weight }
        : null;
    case "external_ip":
      return f.extAddr
        ? { external_ip: { address: f.extAddr, zone_id: f.zoneId ?? "" }, weight }
        : null;
    default:
      return null;
  }
...
export function targetIdentityOnly(t: Target): Target {
  ...
  if (t.external_ip)
    return { external_ip: { address: t.external_ip.address, zone_id: t.external_ip.zone_id } };
  return {};
}
        ...
        >
          {targets.length === 0 ? (
            <Typography.Text type="secondary">
              Targets ещё не добавлены — нажмите «Добавить target».
            </Typography.Text>
          ) : (
            <Table<Target>
  ...
  return (
    <>
      ...
      <Modal
        ...
        destroyOnHidden
      >
        <Form layout="horizontal" labelCol={{ flex: "170px" }} wrapperCol={{ flex: "auto" }} labelAlign="left" colon={false}>
          ...
          {kind === "instance" && (
            <Form.Item label="Instance">
              <RefSelect refResource="compute-instances" refProjectScoped value={form.instanceId} onChange={(v) => set({ instanceId: v || undefined })} />
            </Form.Item>
          )}
          {kind === "nic" && (
            <Form.Item label="NetworkInterface">
              <RefSelect refResource="network-interfaces" refProjectScoped value={form.nicId} onChange={(v) => set({ nicId: v || undefined })} />
            </Form.Item>
          )}
          {kind === "ip_ref" && (
            <>
              <Form.Item label="Subnet">
                <RefSelect refResource="subnets" refProjectScoped value={form.subnetId} onChange={(v) => set({ subnetId: v || undefined })} />
              </Form.Item>
              <Form.Item label="Адрес">
                <Input value={form.ipAddr ?? ""} onChange={(e) => set({ ipAddr: e.target.value.trim() })} placeholder="10.0.0.5" style={{ fontFamily: "monospace" }} />
              </Form.Item>
            </>
          )}
          {kind === "external_ip" && (
            <>
              <Form.Item label="Адрес">
                <Input value={form.extAddr ?? ""} onChange={(e) => set({ extAddr: e.target.value.trim() })} placeholder="203.0.113.10" style={{ fontFamily: "monospace" }} />
              </Form.Item>
          ...
          <Form.Item label="Weight">
            <InputNumber min={0} max={1000} value={form.weight ?? 1} onChange={(v) => set({ weight: typeof v === "number" ? v : 1 })} />
