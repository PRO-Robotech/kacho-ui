...
export function LoginPage() {
  ...
  const hasWebauthn = useMemo(
    () => (flow ? !!findNode(flow.ui, "webauthn_login_trigger") : false),
    [flow],
  );
  const hasPassword = useMemo(
    () => (flow ? !!findNode(flow.ui, "password") : false),
    [flow],
  );
    ...
    try {
      ...
      const cred = (await navigator.credentials.get({
        publicKey: {
          ...opts.publicKey,
          userVerification:
            flow.requested_aal === "aal2" ? "required" : "preferred",
        },
  ...
  return (
    <AuthLayout>
      ...
      <Text type="secondary">
        Используйте passkey или пароль для входа в аккаунт
      </Text>

      {info && (
        <Alert type="info" showIcon message={info} style={{ marginTop: 16 }} />
      )}
      {messages.map((m, i) => (
        <Alert
          key={i}
