...
export function RegisterPage() {
  ...
  const hasWebauthn = useMemo(
    () => (flow ? !!findNode(flow.ui, "webauthn_register_trigger") : false),
    [flow],
  );
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      setHibpWarn(
        pwned
          ? "Этот пароль уже встречался в утечках. Выберите другой."
          : null,
      );
    }, 500);
  ...
  const submitPasskey = async () => {
    ...
    try {
      ...
      const result = await kratos.submitFlow<SelfServiceFlow>(
        "registration",
        flow.id,
        body,
      );
  ...
  return (
    <AuthLayout>
      ...
      <Text type="secondary">
        Создайте аккаунт с passkey или паролем
      </Text>
