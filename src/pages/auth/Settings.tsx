...
export function SettingsPage() {
  ...
  useEffect(() => {
    ...
    kratos
      ...
      .then((f) => {
        ...
        if (f.refresh === true) {
          setInfo(
            "Для изменений безопасности нужна повторная проверка. Подтвердите passkey.",
          );
        }
      })
  ...
  const passkeys = useMemo<PasskeyEntry[]>(() => {
    ...
    return flow.ui.nodes
      ...
      .map((n) => ({
        ...
        display_name:
          (n.meta?.label?.text as string | undefined) ?? "Passkey",
  ...
  const totpImage = useMemo(() => {
    if (!flow) return null;
    const n = flow.ui.nodes.find(
      (x) => x.type === "img" && x.attributes?.name === "totp_qr",
    );
  ...
  const addPasskey = async () => {
    ...
    try {
      ...
      const updated = await kratos.submitFlow<SelfServiceFlow>(
        "settings",
        flow.id,
        body,
      );
  ...
  return (
    <AuthLayout>
      ...
      <Text type="secondary">
        Управление passkey и TOTP в {config.webauthnRpName}
      </Text>
        ...
        {totpEnrolled ? (
          ...
          <>
            {totpImage && (
              <>
                <Paragraph type="secondary">
                  Отсканируйте QR в Google Authenticator / Authy / 1Password:
                </Paragraph>
            ...
            >
              <Form.Item
                name="totp_code"
                rules={[{ required: true, message: "6-значный код" }]}
              >
                <Input
              ...
              <Form.Item>
                <Button
                  ...
                  htmlType="submit"
                  loading={submitting}
                  data-testid="settings-totp-submit"
                >
                  Подтвердить
                </Button>
              </Form.Item>
