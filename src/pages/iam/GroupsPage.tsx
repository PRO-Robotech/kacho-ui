...
export function GroupsPage() {
  ...
  const list = useQuery({
    ...
    queryFn: () =>
      iamApi.listGroups({ account_id: accountId!, pageSize: "200" }),
    enabled: !!accountId,
    ...
    <Space direction="vertical" size={12} style={{ width: "100%" }}>
      ...
      {!accountId ? (
        <Typography.Text type="secondary">
          Выберите Account, чтобы увидеть его Groups.
        </Typography.Text>
      ) : (
        <Table<Group>
          rowKey="id"
          ...
          expandable={{
            expandedRowRender: (row) => (
              <GroupMembersPanel group={row} accountId={accountId} />
            ),
          }}
...
function GroupMembersPanel({
  ...
  const sas = useQuery({
    ...
    queryFn: () =>
      iamApi.listServiceAccounts({ account_id: accountId!, pageSize: "1000" }),
    enabled: !!accountId,
    staleTime: 30_000,
  });
