  ...
  return (
    <div>
      ...
      <div style={{ fontSize: 12, marginTop: 6, color: "rgba(255,255,255,0.45)" }}>
        Один permission на строку. Формат:{" "}
        <code>module.resource.verb</code> (e.g.{" "}
        <code>compute.instances.*</code>).
      </div>
      {invalid.length > 0 && (
        <Alert
          type="error"
      ...
      {lines.length > 0 && invalid.length === 0 && (
        <div style={{ fontSize: 12, marginTop: 6, color: "#52c41a" }}>
          {lines.length} permissions OK
        </div>
      )}
    </div>
  );
    ...
    >
      <Form
        ...
        onFinish={(v) => {
          ...
          const permsChanged =
            perm_list.length !== origPerms.length ||
            perm_list.some((p, i) => p !== origPerms[i]);
          if (permsChanged) {
