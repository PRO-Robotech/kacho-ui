  ...
  return (
    <Space direction="vertical" size={12} style={{ width: "100%" }}>
      ...
      <Space size={8} wrap>
        <Tooltip
          title={
            account ? undefined : "Выберите Account вверху секции, чтобы пригласить пользователя"
          }
        >
          <Button
            type="primary"
      ...
      {users.length === 0 && !isLoading && (
        <Alert
          ...
          description={
            <span>
              Пригласите пользователя по email (кнопка выше) — он получит
              magic-link для активации. Также User создаётся автоматически из
              OIDC-callback Zitadel.
            </span>
          }
        />
      )}
    ...
    >
      {magicLink ? (
        ...
        >
          ...
          >
            Проект и роль необязательны — можно назначить позже через Access
            Bindings.
          </Typography.Paragraph>
        </Form>
      )}
