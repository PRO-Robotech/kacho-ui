...
import { CopyableMonoId, fmtTs } from "@/components/iam/IamCommon";
import { GrantAdminModal } from "@/components/system/GrantAdminModal";
...
export default function ClusterAdminsPage() {
    ...
    },
    {
      ...
      render: (_v, row) => {
        ...
        const tooltip = isSelf
          ? "Нельзя отозвать самого себя"
          : isLast
          ? "Нельзя отозвать последнего admin'а"
          : "";
        const button = (
          <Button
            size="small"
  ...
  return (
    <Space
      ...
      size={16}
      style={{ width: "100%" }}
      data-testid="cluster-admins-page"
    >
      <div>
        <Typography.Title
          level={3}
          style={{ margin: 0 }}
          data-testid="cluster-admins-page-title"
        >
          Cluster admins
        </Typography.Title>
      ...
      {adminsQ.error && !isForbidden && (
        <Alert
          ...
          description={
            adminsQ.error instanceof Error
              ? adminsQ.error.message
              : String(adminsQ.error)
          }
        />
      )}
